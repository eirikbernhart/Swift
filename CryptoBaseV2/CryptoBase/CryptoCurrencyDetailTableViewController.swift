...
class CryptoCurrencyDetailTableViewController: UITableViewController, UITabBarControllerDelegate {
    ...
    @objc func buyTapped() {
        ...
        let buyAction = UIAlertAction(title: "Confirm", style: .default, handler: {(action) in
            ...
            if(Double(sum)! <= 0) {
                print("CAN ONLY ADD POSITIVE VALUES!")
            } else {
    ...
    func getDataFromPersistentStorage()  {
        ...
        do {
            ...
            if (amountOfCC <= 0) {
                deleteFromPersistentStorage(symbol: symbol)
            }
    ...
    func tabBarController(_ tabBar: UITabBarController, didSelect viewController: UIViewController) {
